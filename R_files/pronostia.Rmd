---
title: "features clustering pronostia"
...
init_partition <- read_csv("init_partition/init_partition_pronostia.csv")
init_partition$cluster

X <- read_csv("data_cleaned/X_cleanpronostia.csv")
X <- X[, -1]
# Fit a clv initialization with K'+1 partition or CLV with K'=K
# package ClustVarLV package disponible dans R

res.clvkm.hc <- CLV_kmeans(X = X, method = "directional", clust=init_partition$cluster, strategy="kplusone", iter.max = 500)
